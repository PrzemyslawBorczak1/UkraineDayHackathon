...
Expected interval shape:
    ...
    interval.is_delivery_trip                      -> bool
...
# Engine Status.name -> Task phase. IN_PROGRESS_ACTIVE is split by is_delivery_trip.
_PHASE_BY_STATUS = {
...
def map_phase(status_name: str, is_delivery_trip: bool) -> str:
    """Map an engine interval phase to a Task status."""
    if status_name == "IN_PROGRESS_ACTIVE":
        return TaskStatus.TRANSPORTING if is_delivery_trip else TaskStatus.UNLOAD
...
def task_from_interval(vehicle_id: str, interval) -> Optional[Task]:
    ...
    return Task(
        ...
        status=map_phase(status_name, bool(interval.is_delivery_trip)),
